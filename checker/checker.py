...
def check_linter(filename: str):
    ...
    check_line = f'clang-tidy --config-file=clang-tidy.json -extra-arg=-std=c++20 {filename} -- {extra_compile_flags} 1>tidy_log.txt'
    return_code = subprocess.run(check_line, shell=True).returncode
    if return_code != 0 or os.stat('tidy_log.txt').st_size > 0:
        print("Linter failed")
        subprocess.run("cat tidy_log.txt", shell=True)
